...
`CC_ARCHIVE=1 CC_BATCH=1 CC_BATCH_N=3`，即 4 个序列、`n_ctx` 524288。草稿缓存默认
关闭，所以本节所有测量都是在投机解码关闭的情况下做的。
...
**前缀 archive。** 默认开（`CC_ARCHIVE=0` 关掉）。兄弟 subagent 共享长前缀，但彼此
不是对方的延续，普通前缀缓存帮不上。archive 在请求**真正分叉**的点上保存整个 KV
状态，后来有请求要时整块恢复。上面那次会话里它服务了 20 个请求中的 7 个；最清楚的
一个是 24,775 token 的提示词只转发了 **2,324** 个，另外 22,451 个已经在归档状态里。
...
决定投机划不划算的是**接受率**，不是上下文长度。整份草稿都落地的一轮，大约比逐 token
解码快 3 倍；只落地一部分的一轮，等于两个前向换大约一个 token。所以
`CC_DDC_MAX_CTX` 默认等于该序列自己的窗口，不会提前切断 DDC，只保留"草稿不许越过
上下文末尾"这一个钳制作用。
...
| 变量 | 默认 | 作用 |
|---|---|---|
| `CC_ARCHIVE` | 开 | 前缀 archive。设 `CC_ARCHIVE=0` 关闭 |

**可选：**

| 变量 | 默认 | 作用 |
|---|---|---|
| `CC_DDC` | 关 | 投机解码；设为 `2` 或 `4` 打开，数字是草稿缓存键宽。在 archive 的槽之外再占一个序列 |
| `CC_DDC_BATCH=1` | 关 | 让 DDC 在批处理调度器里投机，而不是在 `CC_BATCH` 下让位。新写的、未运行过 —— 见 DDC 一节 |
...
| `CC_DDC_MAX_CTX` | 窗口长度 | 草稿缓存让开的长度；默认等于该序列自己的窗口，`0` 表示连钳制也去掉 |
...
| `CC_DDC_BATCH_SCRATCH` | 1 | 批处理调度器可用于 DDC 验证的 scratch 序列数。每多一个就多一个请求能同时投机，代价是多占一个序列的 KV |
...
- **轮次键复用**用模型之前对同一个问题的回复来作答。对话被重放、或问题确实重复时
  ...
  问题相似度可以到 0.65，所以下限放得松，就会拿一个问题的答案去回答另一个。下限
  默认 0.9，且差异中没有任何可替换项时会被拒绝。需要每个回复都在当前上下文下产生，
  就把 `CC_QREUSE` 留在 0。
...
- **前缀 archive 默认开，`CC_DDC` 默认关**，和以前正好相反。打开 DDC 要在 archive
  的槽之外再占一个序列 —— archive 留自己的槽，DDC 留自己的验证 scratch，合计 `n_ctx`
  乘以 worker 数 + 1 + archive 槽数。这两个设置谁也不会关掉谁。
